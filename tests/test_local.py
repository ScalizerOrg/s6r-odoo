...
def test_local():
    ...
    odoo = OdooConnection(url='http://odoo_digitalportage.localhost',
                          dbname='digitalportage_prod',
                          user='admin',
                          password='Admin1234!', debug_xmlrpc=True, logger=logger)
    #
    ...
    user_id = odoo.model('res.users').values_to_record({'id': f'external_config.user_test_03',
                                                                   'name': 'Lastname Firstname TEST3',
                                                                   'login': 'test3@test.fr',
                                                                   'country_id/id': 'base.af'})

    odoo.print_query_count()
    user_id.save()
    odoo.print_query_count()
    # user_id = odoo.ref('external_config.user_test_03')
    #                    # , fields=['country_id', 'name'])
    # user_id['country_id/id'] = 'base.al'
    # print(user_id)


    users = odoo.model('res.users').search([('id', 'in', [1, odoo.get_ref('external_config.user_test_03')])])
    if users:
        users[0].unlink()
    print(users)
    odoo.print_query_count()
